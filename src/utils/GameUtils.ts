class GameUtils {
    ...
    public static btnToListener(btn: eui.Button, callback) {
    ...
    // 加载bgm
    public static loadBgm(name: string) {
        const bgm = RES.getRes(name);
        let isPlaying = false;
        let stopBgm: egret.SoundChannel;
        let lastPlayPosition = 0;
        return {
            play: (start = 0, loop = 1) => {
                if (!isPlaying) {
                    stopBgm = bgm.play(start || lastPlayPosition, loop || 0);
                    isPlaying = true;
                };
            },
            stop: () => {
                lastPlayPosition = stopBgm.position;
                isPlaying = false;
                stopBgm && stopBgm.stop();
            },
        }
    }
}